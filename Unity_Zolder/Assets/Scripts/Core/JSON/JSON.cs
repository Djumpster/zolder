...
namespace Talespin.Core.Foundation.Parsing
{
	public partial class JSON
	{
		...
		/// <summary>
		/// On encoding, this value holds the key of the last value that could not be serialized (null = no error).
		/// </summary>
		...
		public static object JsonDecode(string json)
		{
			...
			if (json != null)
			{
				...
				{
					LogUtil.Error(LogTags.DATA, "JSON", "Error parsing string at index: " + index + " ,line: " + currentLine);
					LogUtil.Error(LogTags.DATA, "JSON", json);
					instance.lastErrorIndex = index;
					instance.lastErrorLine = currentLine;
				}
				return value;
			}
			else
			{
				throw new System.ArgumentNullException("Json input string is null!");
			}
		}
		...
		public static string JsonEncode(object json)
		{
			...
			{
				if (instance.lastErrorKey == null)
				{
					instance.lastErrorKey = string.Empty;
				}
		...
		/// <summary>
		/// This function returns whether an error occured during the last decoding operation.
		/// </summary>
		/// <returns></returns>
		...
		/// <summary>
		/// This function returns whether an error occured during the last encoding operation.
		/// </summary>
		/// <returns></returns>
		public static bool LastEncodeSuccessful()
		{
			return (instance.lastErrorKey == null);
		}
		...
		public static string GetLastErrorSnippet()
		{
			...
			{
				int startIndex = instance.lastErrorIndex - 45;
				int endIndex = instance.lastErrorIndex + 45;
				if (startIndex < 0)
				{
					startIndex = 0;
				}
		...
		/// <summary>
		/// This function returns the key of the entry that caused an error during the last encoding operation.
		/// </summary>
		/// <returns></returns>
		public static string GetLastErrorKey()
		{
			return instance.lastErrorKey;
		}
		...
		protected double ParseNumber(char[] json, ref int index)
		{
			...
			index = lastIndex + 1;
			// IMPORTANT! large integers will be rounded due to insufficient precision of float
		...
		protected void EatWhitespace(char[] json, ref int index)
		{
			for (; index < json.Length; index++)
			{
				...
				//if (" \t\n\r".IndexOf(json[index]) == -1 && (int) json[index] != 65279)
				if (!char.IsWhiteSpace(json[index]) && json[index] != 65279)
				{
					break;
				}
			}
		...
		{
			// only append the generated string if this serialization step succeeds.
			...
			while (e.MoveNext())
			{
				...
				SerializeString(key, objectString);
				objectString.Append(":");
				if (!SerializeValue(value, objectString))
				{
					lastErrorKey = key;
					return false;
				}
		...
		protected bool SerializeArray(ArrayList anArray, StringBuilder builder)
		{
			...
			for (int i = 0; i < anArray.Count; i++)
			{
				...
				if (!SerializeValue(value, objectString))
				{
					lastErrorKey = string.Format("array_value[{0}]", i);
					return false;
				}
		...
		/// (can be integer, double, etc). 
		/// </summary>
		protected bool IsNumeric(object o)
		{
			double f;
			return double.TryParse(o.ToString(), out f);
		}
	}
}