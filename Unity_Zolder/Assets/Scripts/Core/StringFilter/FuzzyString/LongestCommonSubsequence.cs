...
namespace Talespin.Core.Foundation.Filter
{
	public static partial class ComparisonMetrics
	{
		public static string LongestCommonSubsequence(this string source, string target)
		{
			...
			return LongestCommonSubsequence(source, target, matches);
		}

		public static string LongestCommonSubsequence(this string source, string target, List<Tuple<int, int>> matches)
		{
		...
		private static int[,] LongestCommonSubsequenceLengthTable(string source, string target)
		{
			...
			for (int i = 1; i < source.Length + 1; i++)
			{
				for (int j = 1; j < target.Length + 1; j++)
				{
					...
					{
						C[i, j] = Mathf.Max(C[i, j - 1], C[i - 1, j]);
					}
				}
			}
